---
title: "Claude Certified Architect — Professional (CCAR-P)"
description: "Advanced professional-level certification covering enterprise multi-agent deployment, system safety, and scalable agentic patterns."
layout: "single"
---

<div class="card" style="border-left: 6px solid #d97706; background: rgba(217, 119, 6, 0.05); padding: 1.5rem; margin-bottom: 2rem;">
  <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
    <span class="badge" style="background: #d97706; color: #fff;">PLANNED CERTIFICATION</span>
    <span class="muted">Status: In Preparation</span>
  </div>
  <h2 style="margin: 0.5rem 0;">CCAR-P Practice Library Coming Soon</h2>
  <p>The <strong>Claude Certified Architect — Professional (CCAR-P)</strong> certification exam materials, scenario question bank, and timed mock test engine are currently being curated by the certification team.</p>
</div>

## Exam Overview

* **Provider:** Anthropic
* **Level:** Professional
* **Format:** 60 Scenario-Based Questions / 120 Minutes
* **Passing Score:** 72% (720 / 1000)
* **Estimated Exam Fee:** $250 USD

---

## What to Do Next

While the CCAR-P material is being finalized, prepare for the **Foundations (CCAF)** exam first:

* 👉 <a href="/exams/cca-f/"><strong>CCAF Exam Overview & Study Center</strong></a>
* 📝 <a href="/exams/cca-f/sample-questions/"><strong>CCAF 200+ Practice Question Library</strong></a>
* ⏱️ <a href="/exams/cca-f/mock-test/"><strong>CCAF 60Q/120M Timed Mock Test Engine</strong></a>
* 📚 <a href="/exams/cca-f/study-materials/"><strong>CCAF Official Documents & PDFs</strong></a>
